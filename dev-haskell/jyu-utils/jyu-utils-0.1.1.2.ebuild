# Copyright 1999-2015 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI=5

# ebuild generated by hackport 0.4.5

CABAL_FEATURES="lib profile haddock hoogle hscolour"
inherit haskell-cabal

MY_PN="JYU-Utils"
MY_P="${MY_PN}-${PV}"

DESCRIPTION="Some utility functions for JYU projects"
HOMEPAGE="http://hackage.haskell.org/package/JYU-Utils"
SRC_URI="mirror://hackage/packages/archive/${MY_PN}/${PV}/${MY_P}.tar.gz"

LICENSE="MIT"
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/binary-0.5:=[profile?]
	>=dev-haskell/lazysmallcheck-0.5:=[profile?]
	>=dev-haskell/mtl-1.1.0:=[profile?]
	>=dev-haskell/mwc-random-0.8:=[profile?] <dev-haskell/mwc-random-0.9:=[profile?]
	>=dev-haskell/parallel-1.0:=[profile?]
	>=dev-haskell/quickcheck-2.1:2=[profile?]
	>=dev-haskell/random-1.0.0:=[profile?]
	>=dev-haskell/stm-2.1:=[profile?]
	>=dev-haskell/zlib-0.5:=[profile?]
	>=dev-lang/ghc-7.4.1:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-1.4
"

S="${WORKDIR}/${MY_P}"
