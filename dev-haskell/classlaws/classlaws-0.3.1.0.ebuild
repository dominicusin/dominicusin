# Copyright 1999-2015 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI=5

# ebuild generated by hackport 0.4.5

CABAL_FEATURES="lib profile haddock hoogle hscolour"
inherit haskell-cabal

MY_PN="ClassLaws"
MY_P="${MY_PN}-${PV}"

DESCRIPTION="Stating and checking laws for type class methods"
HOMEPAGE="http://wiki.portal.chalmers.se/cse/pmwiki.php/FP/ClassLaws"
SRC_URI="mirror://hackage/packages/archive/${MY_PN}/${PV}/${MY_P}.tar.gz"

LICENSE="BSD"
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/chasingbottoms-1.3:=[profile?] <dev-haskell/chasingbottoms-2:=[profile?]
	>=dev-haskell/mtl-1:=[profile?] <dev-haskell/mtl-3:=[profile?]
	>=dev-haskell/quickcheck-2:2=[profile?] <dev-haskell/quickcheck-2.5:2=[profile?]
	>=dev-lang/ghc-7.4.1:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-1.8
"

S="${WORKDIR}/${MY_P}"
