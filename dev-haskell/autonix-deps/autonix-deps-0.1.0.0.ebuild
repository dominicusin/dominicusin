# Copyright 1999-2015 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI=5

# ebuild generated by hackport 0.4.5

CABAL_FEATURES="lib profile haddock hoogle hscolour"
inherit haskell-cabal

DESCRIPTION="Library for Nix expression dependency generation"
HOMEPAGE="http://hackage.haskell.org/package/autonix-deps"
SRC_URI="mirror://hackage/packages/archive/${PN}/${PV}/${P}.tar.gz"

LICENSE="BSD"
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/conduit-1.2:=[profile?]
	>=dev-haskell/errors-1.4:=[profile?]
	>=dev-haskell/lens-4.0:=[profile?]
	>=dev-haskell/libarchive-conduit-0.1:=[profile?] <dev-haskell/libarchive-conduit-0.2:=[profile?]
	>=dev-haskell/mtl-2.1:=[profile?]
	>=dev-haskell/optparse-applicative-0.11:=[profile?]
	>=dev-haskell/regex-tdfa-1.2:=[profile?]
	>=dev-haskell/resourcet-1.1:=[profile?]
	>=dev-haskell/transformers-0.3:=[profile?]
	>=dev-haskell/xml-1.3:=[profile?]
	>=dev-lang/ghc-7.8.2:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-1.18.1.3
"
